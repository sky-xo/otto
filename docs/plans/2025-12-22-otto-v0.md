# June V0 Implementation Plan

> **For Claude:** Use `superpowers:subagent-driven-development` to implement this plan. Dispatch fresh subagent per task, with spec compliance review then code quality review after each.

---

## CRITICAL: Keeping This Plan Updated

**This plan file is your source of truth.** When auto-compaction happens, you lose conversation context. This file is how you recover and know exactly where to continue.

### After EVERY task completion:
1. Update the **Current Status** section below immediately
2. Commit the plan update: `git add docs/plans/2025-12-22-june-v0.md && git commit -m "docs: update plan status after Task N"`

### What to update:
- **Last completed:** Which task just finished (include review status)
- **In progress:** What's currently being worked on (or "None")
- **Blockers:** Any issues preventing progress
- **Notes:** Key context (commit SHAs, review findings, decisions made)
- **Next:** Exactly what task to do next

### If you lose context (compaction happened):
...
5. Continue from where **Next** says to go

### Review cycle reminder:
For each task: Implement → Spec review (passes?) → Code quality review (passes?) → Update plan → Next task

---

## Current Status

- **Last completed:** Task 10 (june watch --ui - Bubbletea TUI) - ALL TASKS COMPLETE
- **In progress:** None
- **Blockers:** None
- **Notes:** V0 Seed + TUI enhancement complete! All 10 tasks done. June is ready to use.
- **Next:** Ready for testing and use

---

### Task 1a: Fix Task 1 quality issues (required before Task 2)

Code quality review found 2 Important issues in Task 1 that need fixing:

**Issue 1: Error handling in Execute() - internal/cli/root.go**

Current code silently discards the error:
```go
func Execute() {
    rootCmd := &cobra.Command{Use: "june"}
    _ = rootCmd.Execute()  // Error is silently discarded
}
```

Fix to handle error properly:
```go
func Execute() {
    rootCmd := &cobra.Command{Use: "june"}
...
**Issue 2: Incomplete test coverage - internal/config/paths_test.go**

Only one test case exists. `DataDir()` has 3 code paths but only 1 is tested:
1. ✅ When `HOME` env var is set (tested)
2. ❌ When `HOME` is not set but `os.UserHomeDir()` succeeds (not tested)
3. ❌ When both fail and falls back to `.june` (not tested)

Add at least one more test case for the fallback path.

**After fixing:** Re-run code quality review to confirm issues resolved, then proceed to Task 2.

---

**Goal:** Ship a Seed/V0 `june` CLI that can spawn agents, persist state in SQLite, and support core messaging workflows without daemon or worktree support.

**Architecture:** A Go CLI (Cobra) writes/reads from a per-orchestrator SQLite DB in `~/.june/` (versionless for V0) and shells out to `claude`/`codex` for agent execution. Orchestrator scope is derived from git repo + branch, and all state is managed through small repository helpers.
...
```bash
go mod tidy
git add go.mod go.sum cmd/june/main.go internal/cli/root.go internal/config/paths.go internal/config/paths_test.go
...
### Task 6: Implement `june say`, `june ask`, `june complete`
...
### Task 7: Implement `june messages` + `june status`
...
### Task 8: Implement `june spawn`, `june prompt`, `june attach`
...
### Task 9: Implement `june watch` (simple polling)
...
Plan complete and saved to `docs/plans/2025-12-22-june-v0.md`.
